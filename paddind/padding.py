...
while f.readline():
    ...
    img_name=f.readline().replace('remap','remap_padding')[:-1]
    print(img_name)
    img=np.array(Image.open(f.readline()[:-1]))
    img_padding=preProcessPadding(img)
    if not os.path.exists(os.path.split(img_name)[0]):
        os.makedirs(os.path.split(img_name)[0])
    cv2.imwrite(img_name,img_padding*255.0)
    print(1)