...
dataDir = '/data/ICE/coco/annotations'  ####### 连接到coco的数据集
headstr = """\
<annotation>
    <folder>VOC</folder>
    <filename>%s</filename>
...
def save_annotations_and_imgs(coco, dataset, filename, objs):
    ...
    img_path = os.path.split(dataDir)[0] +'/'  + dataset + '/' + filename
...
def showimg(coco, dataset, img, classes, cls_id, show=True):
    ...
    I=Image.open('%s/%s/%s'%(os.path.split(dataDir)[0],dataset,img['file_name']))  ########may be you can changed
