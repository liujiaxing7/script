...
from to_onnx.ssd.utils.nms import boxes_nms
...
class PostProcessor:
    ...
    def __call__(self, detections):
        ...
        for batch_id in range(batch_size):
            ...
            # score =  per_img_scores.softmax(dim=1)
            # score[:, 0] = 0
            # score, labels = score.max(1)         # [#anchors,]
            # ids = score > self.cfg.TEST.CONFIDENCE_THRESHOLD
            # # ids = ids.nonzero().squeeze()             # [#obj,]
            # keep = boxes_nms(per_img_boxes[ids], score[ids], self.cfg.TEST.NMS_THRESHOLD, self.cfg.TEST.MAX_PER_CLASS)
            # processed_scores = score[ids][keep]
            # processed_boxes = per_img_boxes[ids][keep]
            # processed_labels = labels[ids][keep]
