...
def train_test_val_split_by_files(img_paths, root_dir):
    ...
    return img_split[0], img_split[1]
...
def yolo2coco(arg):
    ...
    if arg.random_split or arg.split_by_file:
        ...
        if arg.random_split:
            ...
            train_img, val_img = train_test_val_split_by_files(indexes, root_path)
    else:
    ...
    for k, index in enumerate(tqdm(indexes)):
        # 支持 png jpg 格式的图片。
        txtFile = index.replace('JPEGImages', 'txt').replace('.jpg', '.txt').replace('.png', '.txt').replace('.JPEG', '.txt')
        # 读取图像的宽和高
